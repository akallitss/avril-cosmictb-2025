...
//Analysis constant
const double zdet = 666+25;//mm in ray referencial 
...
vector <double> * fovclustamp,* fovclustmaxamp,  * fovclusttime,  * fovclustx;
...
void DoAnalysis(int maxevn){
  ...
  for(int iev=0; iev<nev;iev++){
    ...
    for(int iray=0;iray<rayN;iray++)
      {	
	fovdetx->clear(); fovdetamp->clear(); fovdettime->clear();
	fovclustx->clear(); fovclustamp->clear();fovclustmaxamp->clear(); fovclusttime->clear(); fovclustsize->clear(); 
	double clustx = 0., clustamp=0., clustmaxamp=0., clusttime=0.; int clustsize = 0, clustlaststrip = 0;
	  ...
	  for(int istr=0;istr<fNstrip;istr++)//max = fNdet
	      ...
	      if(val>=nsigma*RMS[istr+fNstrip*idet])//ZS
		{
		  ...
		  if(clustsize==0)//creating cluster
			...
			{
			  clustamp+=val;
			  if(val>clustmaxamp) clustmaxamp=val;
			  clusttime+=time;
			  clustsize++;
			  clustx+=getXdet(idet,istr)*val;
			  clustlaststrip = istr;
