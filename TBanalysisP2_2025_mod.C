...
map<int, mp2> vmp2; //map containing all mapping
...
void DoAnalysis(int maxevn){
  ...
  cout << "Tracking entries: " << nev << endl;
  cout << "P2 entries: " << nevd << endl;
  ...
  treRay->GetEntry(0); treData->GetEntry(0);
  int firstevn = evn; int firstevndata = fevnData;
  cout << "first ray evn : " << firstevn << " first data evn : " <<  firstevndata << endl;

  int minEvn = firstevn;
  for(int iev=0; iev<nev;iev++){
    treRay->GetEntry(iev);
    if(evn < minEvn){minEvn=evn;}
  }
   
  for(int iev=0; iev<nev;iev++){
    treRay->GetEntry(iev);
    treData->GetEntry(evn - minEvn);//iev
    //if(iev < 100)cout << "iev : " << iev << " vs evn-minEvn : "<< evn-minEvn << endl;
  ...
  fovclustx->clear();fovclusty->clear(); fovclustamp->clear();fovclustmaxamp->clear(); fovclusttime->clear(); fovclustmaxstrip->clear(); fovclustmaxstripx->clear(); fovclustmaxstripy->clear();
	...
	double rayxprime = rayx*cos(theta)-rayy*sin(theta) +0;
	double rayyprime = rayx*sin(theta)+rayy*cos(theta) +0;
...
fovstrip->push_back(istr);
    ...
    fovdetch->push_back(channel);//contains connector mapping
      ...
      //cout << "idet : "<<idet << endl;
       ...
       cluster(fovdetch, fovdetx, fovdety, fovdetamp, fovclustamp, fovclustsize, fovclustmaxamp, fovclustmaxstrip, fovclustx, fovclusty, fovclustmaxstripx, fovclustmaxstripy);
  fovlargestclust=(*max_element(fovclustamp->begin(), fovclustamp->end()));
...
//---------------------------------------------------
//-----------MAIN------------------------------------
void TBanalysisP2_2025_mod(string folder="", int maxevn = -1) {
