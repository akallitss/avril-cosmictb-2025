...
const double XDET[] = {1.70886e+02-1.07499e+00,//TPOT
		       ...
		       3.98319299999999998e+01,//RD3 mid
...
void OuttreeAna(TString filename, int maxev = -1)
{
  ...
  for(int iev=0; iev<nev;iev++){
	...
	{ard3[detn-1]=clustmaxamp->at(icl); xrd3[detn-1]=clustx->at(icl)*SCALE[detn-1];}
    }//cluster loop
    
    double Ex=0, Ez=0, Ex2=0, Exz=0; bool trok=true;
    for(int id=0;id<nrd3;id++) 
      {
	//least sq method //TODO CHECK
	if(ard3[id]<0) {trok=false;break;}
	Ex += xrd3[id];
	Ez += ZDET[id+1];
	Exz += xrd3[id]*ZDET[id+1];
	Ex2 += xrd3[id]*xrd3[id];
      } 
    if(trok)
      {
	double m=(nrd3*Exz-Ex*Ez)/(nrd3*Ex2-Ex*Ex);
	double p=(Ez-m*Ex)/nrd3;
	double xextra = m*ZDET[DETEXTRA]+p;
	fovextra->push_back(xextra-xrd3[DETEXTRA-1]);
      }
    finaltree->Fill();
  }//end for events
  finaltree->Write();
