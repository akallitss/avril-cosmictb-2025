...
void efficacite() {
   ...
   // --- Opening the file ---
   TFile *f = new TFile("/data/cosmic_data/2025/W43/workdir_CosTb_P2_mapping_M400V_v2/outtree_aligned.root");
   if (!f || f->IsZombie()) {
      cout << "Can't open outtree_aligned.root" << endl;
      return;
   }
   ...
   map<int,int> ChActive;
   map<int,int> passed;
   map<int,int> seen;

   TH2D *hRayPassed = new TH2D(
      "hRayPassed",";RayX_{corr};RayY_{corr}",
      100, 60, 600,
      100, 0, 540
   );

   TH2D *hRaySeen = new TH2D(
      "hRaySeen",";RayX_{corr};RayY_{corr}",
      100, 60, 600,
      100, 0, 540
   );
...
//       }
//    }
//    cout << "taille rms : " << RMSval.size() << endl;
//   for(std::map<int,double>::iterator it = RMSval.begin(); it != RMSval.end(); ++it){
//    int ch = it->first;
//    double rms = it->second;

//    if(nbChan[ch] > 0){
//       hRMS->Fill(rms / nbChan[ch]);
//       if ((rms / nbChan[ch])<2) cout << ch << " " ;
//    }
   
//    }
//    cout << endl;

   int rayon = 30; //accptence radius for efficiency
   ...
   for (Long64_t iev = 0; iev < N; iev++) {
      ...
      //cout << minDist << endl;
      passed[ch]++;
      hRayPassed->Fill(RayX_corr, RayY_corr);
      ...
      for (int h = 0; h < nhits; h++) {
         ...
         double dety = DetY->at(h);
		   int detCh = DetCh->at(h); ChActive[detCh]++;
    	   double distance = sqrt((RayX_corr-detx)*(RayX_corr-detx) + (RayY_corr-dety)*(RayY_corr-dety));
		   if(distance<rayon) {seen[ch]++;hRaySeen->Fill(RayX_corr, RayY_corr);}
      }
   }
   ...
   int idmax = -1;
   for (std::map<int,int>::const_iterator it = ChActive.begin();it != ChActive.end(); ++it){
      int value = it->second;
	   int id = it->first;
      if (value < minVal) minVal = value;
      if (value > maxVal) {maxVal = value; idmax = id;}
   }
...
TCanvas *c3 = new TCanvas("c3","c3", 1000, 1000);
   ...
   TH2D *hP2_Pad = new TH2D("hP2_Pad","hP2_Pad",540,60,600,540,0,540);
   
   hP2_Pad->SetStats(kFALSE);
   hP2_Pad->SetMinimum(0);
   hP2_Pad->SetMaximum(maxVal);
   hP2_Pad->GetZaxis()->SetTitle("Number of hits"); 

   TPolyLine **P2_Pad = new TPolyLine *[NCHANNELS_P2];

   hP2_Pad->SetBinContent(1, 1, 0.0001);
   hP2_Pad->Draw("COLZ");
   hP2_Pad->SetContour(100); 

   for(int i=0; i<NCHANNELS_P2; i++){

      double b = 1.0;
      
      vector<double> Pad_coord_X; 
   Pad_coord_X.push_back(-(5+b));
   Pad_coord_X.push_back( (5+b));
   Pad_coord_X.push_back( (5+b));
   Pad_coord_X.push_back(-(5+b));
   Pad_coord_X.push_back(-(5+b));

      vector<double> Pad_coord_Y;
   Pad_coord_Y.push_back(-(5+b));
   Pad_coord_Y.push_back(-(5+b));
   Pad_coord_Y.push_back( (5+b));
   Pad_coord_Y.push_back( (5+b));
   Pad_coord_Y.push_back(-(5+b));
      ...
      P2_Pad[i] = new TPolyLine(5, &Pad_coord_X[0], &Pad_coord_Y[0]);
      
      P2_Pad[i]->SetLineColor(kBlack);
      P2_Pad[i]->SetLineWidth(1);
     
	
	   //cout << ChActive.size() << endl;
      double norm = 0;
	   norm = (double(ChActive[i])) / (maxVal);
	   if(norm == 0) cout << "channel " << i << " innactive" << endl;
      int color = TColor::GetColorPalette(int(norm * (gStyle->GetNumberOfColors()-1)));
      P2_Pad[i]->SetFillColor(color);
...
TCanvas *c4 = new TCanvas("c4","c4", 1000, 1000);
   ...
   if (val < 0.05) deadChn.push_back(i);
...
cout << "eff moy par pad = " << valmoy/NCHANNELS_P2 << endl;

cout << deadChn.size() << "channels with efficiency < 0.05 : ";
for(int i = 0; i < deadChn.size(); i++){
   cout << deadChn[i] << " ";
   }
cout << endl;

//-------------------2D efficiency historgram-----------
...
// for(int ix=1; ix<=hRayEff->GetNbinsX(); ix++){
//   for(int iy=1; iy<=hRayEff->GetNbinsY(); iy++){
//    if(hRayEff->GetBinContent(ix,iy)!=0){
//   sum += hRayEff->GetBinContent(ix,iy);
//   nb++;
// }
//   }
// }
...
// double mean = sum/nb;
// cout << "eff moy = " << mean << endl;

// TCanvas *crms = new TCanvas("crms","RMS",1000,1000);
// hRMS->Draw();



//     // Crée un histogramme
//     TH1F *h = new TH1F("h", "RMS", 200, 0, 10);
