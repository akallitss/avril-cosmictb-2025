...
vector <mp2> vmp2; //vector containing all mapping
...
void ReadMapping(string folderMapping){
  ...
  for(int b=0; b<10; b++)
    {
      char connf[20]; sprintf(connf, "/connector_%d.txt",b);
	...
	while (std::getline(inrms, line)) {
	  ...
	  if(nv>=7) {
	    ...
	    currpad.chan = Channel;
	    currpad.X = X;
	    currpad.Y = Y;
	    currpad.R = Radius;
	    currpad.Phi = Phi;
	    vmp2.push_back(currpad);
	  }
	}
...
void DoAnalysis(int maxevn){
  ...
  for(int iev=0; iev<nev;iev++){
    ...
    for(int iray=0;iray<rayN;iray++)
      {	
	fovdetx->clear(); fovdety->clear();fovdetr->clear();fovdetphi->clear();
...
//---------------------------------------------------
//-----------MAIN------------------------------------
void TBanalysisP2_2025(string folder="", int maxevn = -1) {//todo args...
